...
class QnaController {
  ...
  AddBookMark = async (req, res, next) => {
    ...
    const { userName } = req.decoded;
    try {
      await this.qnaService.AddBookMark(qna_id, userName);
      return res
        .status(201)
  ...
  RemoveBookMark = async (req, res, next) => {
    ...
    try {
      await this.qnaService.RemoveBookMark(qna_id, userName);
      return res
        .status(200)
  ...
  LikeQna = async (req, res, next) => {
    ...
    const { userName } = req.decoded;
    try {
      await this.qnaService.LikeQna(qna_id, userName);
  ...
  RemoveLikeQna = async (req, res, next) => {
    ...
    const { userName } = req.decoded;
    try {
      await this.qnaService.RemoveLikeQna(qna_id, userName);
